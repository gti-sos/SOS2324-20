...
function API_AFO(app) {
    ...
    app.put(API_BASE + "/", (req, res) => {
        ...
        // Busca el objeto en el array de datos
        let index = datos.findIndex(item => JSON.stringify(item) === JSON.stringify(data));
    
        // Si no se encuentra el objeto, añade el nuevo objeto al array y devuelve una respuesta de éxito
        if (index === -1) {
            datos.push(data);
            return res.status(200).send('Recurso añadido con éxito');
        } else {
            // Si el objeto ya existe, devuelve un error
            return res.status(409).send('El recurso ya existe');
        }
    });
    //DELETE 1
